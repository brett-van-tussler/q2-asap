...
config = Config(
    executors=[
        ThreadPoolExecutor(label="default", max_threads=max(psutil.cpu_count() - 0, 1)),
        HighThroughputExecutor(label="htex", max_workers=max(psutil.cpu_count() - 1, 1), provider=LocalProvider()),
    ],
