...
TARGET_FUNCTIONS = ("species ID", "strain ID",
                    "resistance type", "virulence factor")
...
class Target(object):
    ...
    def __init__(self, function, gene_name=None, start_position=None,
                 end_position=None, reverse_comp=False, amplicon=None):
        '''
        Constructor
        '''
    ...
    def __str__(self):
        ...
        if self.gene_name:
            ...
            if self.start_position:
                ...
                output += " (:%d-%d %s" % (self.start_position,
                                           self.end_position, direction)
            output += ")"
...
class Amplicon(object):
    ...
    def __init__(self, sequence, variant_name=None, significance=None,
                 snp=None, regionofinterest=None, AND=None):
        '''
        Constructor
        '''
...
class SNP(object):
    ...
    def __init__(self, position, reference, variant=None,
                 name=None, significance=None):
        '''
        Constructor
        '''
    ...
    def __str__(self):
        return "SNP: %d%s->%s = %s" % (self.position, self.reference,
                                       self.variant, self.significance)
...
class RegionOfInterest(object):
    ...
    def __init__(self, position_range, aa_sequence=None, nt_sequence=None,
                 mutations=None, name=None, significance=None):
        '''
        Constructor
        '''
...
class AND(object):
    ...
    def __init__(self, significance, name=None, target=None,
                 snp=None, regionofinterest=None):
        '''
        Constructor
        '''
        self.significance = significance
    ...
    def __str__(self):
        return "AND: {" + ", ".join(
            map(str, self.operands)) + "} = %s" % self.significance
    ...
    def add_operand(self, value):
        if not (isinstance(value, Target) or isinstance(value, SNP) or
...
def _json_decode(json_dict):
    ...
    json_dict = dict(
        (k.lower() if k != 'AND' else 'AND', v) for k, v in json_dict.items())
...
def _json_encode(obj):
    ...
    if isinstance(obj, Target):
        ...
        if obj.amplicons:
            target_dict["Amplicon"] = (
                obj.amplicons
                if len(obj.amplicons) > 1
                else obj.amplicons[0])
        return target_dict
    if isinstance(obj, Amplicon):
        ...
        if obj.ROIs:
            amplicon_dict["RegionOfInterest"] = (
                obj.ROIs
                if len(obj.ROIs) > 1
                else obj.ROIs[0])
...
def parseJSON(json_fp):
    with open(json_fp, 'r') as file_object:
        ...
        if assay.AND:
            for operand in assay.AND:
                if isinstance(operand, Target):
                    name = (
                        name + "_%s" % operand.gene_name
                        if operand.gene_name
                        else name)
                    for amplicon in operand.amplicons:
                        name = (name + "_%s" % amplicon.variant_name
            ...
            for amplicon in assay.target.amplicons:
                name = (assay.name + "_%s" % amplicon.variant_name
                        if amplicon.variant_name
                        else name)
                seq = DNA(amplicon.sequence, {'id': name})
                yield seq
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='backdoor command to generate a reference fasta '
                    'from an ASAP JSON file',
        epilog='', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('json', type=argparse.FileType('r'),
                        help='JSON file to print as a fasta')
    args = parser.parse_args()

    data = json.load(args.json, object_hook=_json_decode)
    skbio.io.registry.write(generateReference(data['assay']),
                            "fasta", sys.stdout)
